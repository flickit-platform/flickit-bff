...
def create_answer_range(request, kit_version_id):
    response = requests.post(
        ASSESSMENT_URL + f'assessment-core/api/kit-versions/{kit_version_id}/answer-ranges',
        json=request.data,
        headers=AuthHeaderProvider(request).get_headers())
...
def create_answer_option_in_answer_range(request, kit_version_id):
    response = requests.post(
...
def create_measures(request, kit_version_id):
    response = requests.post(
        ASSESSMENT_URL + f'assessment-core/api/kit-versions/{kit_version_id}/measures',
        params=request.query_params,
        json=request.data,
